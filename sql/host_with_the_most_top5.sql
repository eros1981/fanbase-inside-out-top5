...
WITH content AS (
  SELECT 
    ...
    l.user_id,
    u.name AS user_name 
  FROM `758470639878.fanbase-reporting.livestreams` l 
  JOIN `758470639878.fanbase-reporting.users` u ON u.id = l.user_id 
  ...
  FROM `758470639878.fanbase-reporting.voice_channels` v 
  JOIN `758470639878.fanbase-reporting.users` u2 ON u2.id = v.user_id 
