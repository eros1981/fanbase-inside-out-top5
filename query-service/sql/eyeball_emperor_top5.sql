...
WITH views AS (
  SELECT created_at, user_id, views_count FROM `758470639878.reporting_db.flickz`
  UNION ALL
  SELECT created_at, user_id, views_count FROM `758470639878.reporting_db.posts`
  UNION ALL
  SELECT created_at, user_id, views_count FROM `758470639878.reporting_db.livestreams` 
...
active_monthly_views AS (
  ...
  JOIN `758470639878.reporting_db.users` u ON u.id = mv.user_id
